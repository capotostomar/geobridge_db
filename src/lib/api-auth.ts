...
const demoKeys = new Map<string, { id: string; name: string; permissions: string; userId: string; requestCount: number; createdAt: string }>()
...
export async function generateApiKey(name: string, permissions = 'read', userId?: string): Promise<string> {
  ...
  if (isDemoMode()) {
    demoKeys.set(hashedKey, { id: crypto.randomUUID(), name, permissions, userId: userId ?? 'demo', requestCount: 0, createdAt: new Date().toISOString() })
    return rawKey
  }

  const supabase = await createClient()
  await supabase.from('api_keys').insert({ key: hashedKey, name, permissions, active: true, user_id: userId })
  return rawKey
}
...
export async function listApiKeys() {
  if (isDemoMode()) {
    return Array.from(demoKeys.entries()).map(([hashed, v]) => ({
      id: v.id, name: v.name, permissions: v.permissions, active: true,
      lastUsedAt: null, requestCount: v.requestCount, createdAt: v.createdAt,
  ...
  return data.map((k: Record<string, unknown>) => ({
    id: k.id, name: k.name, permissions: k.permissions, active: k.active,
...
export async function revokeApiKey(id: string): Promise<void> {
  if (isDemoMode()) {
    for (const [k, v] of demoKeys.entries()) { if (v.id === id) { demoKeys.delete(k); break } }
    return
  }
  const supabase = await createClient()
...
export async function validateApiKey(req: NextRequest): Promise<{
  valid: boolean; keyName?: string; permissions?: string; userId?: string; error?: string
}> {
  ...
  if (!rawKey) return { valid: false, error: 'Missing API key. Use Authorization: Bearer <key> or ?api_key=<key>' }
  ...
  try {
    const supabase = await createClient()
    const { data: apiKey, error } = await supabase.from('api_keys').select('*').eq('key', hashedKey).single()
    ...
    return { valid: true, keyName: apiKey.name, permissions: apiKey.permissions, userId: apiKey.user_id }
  } catch {

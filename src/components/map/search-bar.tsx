    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      <div className="relative">
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
          {query && (
            <Button variant="ghost" size="icon" onClick={clearSearch} className="h-10 w-10 rounded-full hover:bg-slate-100">
              <X className="w-4 h-4 text-slate-500" />
            </Button>
          )}
          {loading && <Loader2 className="w-5 h-5 animate-spin text-emerald-500 mr-2" />}
        </div>
      </div>
          ...
          {results.map((result, idx) => (
            <button key={idx} onClick={() => handleSelect(result)}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-900 truncate">{result.display_name.split(',')[0]}</p>
                <p className="text-xs text-slate-500 truncate">{result.display_name.split(',').slice(1, 3).join(',')}</p>
              </div>
            </button>
          ))}
        </div>

...
const MapComponent = dynamic(
  () => import('@/components/map/map-component').then(m => m.MapComponent),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-full bg-[#0d1421] flex items-center justify-center">
...
function ProcessingOverlay({ open, title }: { open: boolean; title: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/80 backdrop-blur-sm">
      <div className="bg-[#0d1421] border border-white/10 rounded-2xl p-8 w-full max-w-sm shadow-2xl">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-10 h-10 relative flex-shrink-0">
            <div className="absolute inset-0 rounded-full border-2 border-[#2dd4bf]/20" />
            <div className="absolute inset-0 rounded-full border-2 border-[#2dd4bf] border-t-transparent animate-spin" />
            <Satellite className="absolute inset-0 m-auto w-4 h-4 text-[#2dd4bf]" />
          </div>
          <div>
        ...
        <div className="space-y-2">
          {steps.map((s, i) => (
            <div key={i} className={`flex items-center gap-2 text-xs transition-all duration-300 ${i <= idx ? 'opacity-100' : 'opacity-20'}`}>
              <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 border ${i < idx ? 'bg-[#2dd4bf] border-[#2dd4bf]' : i === idx ? 'border-[#2dd4bf] bg-[#2dd4bf]/20' : 'border-white/15'}`}>
                {i < idx && <span className="text-slate-900 text-[8px] font-bold">✓</span>}
              </div>
              <span className={i <= idx ? 'text-slate-300' : 'text-slate-600'}>{s}</span>
            </div>
          ))}
        </div>
      </div>
...
function AnalysisModal({ open, drawnArea, address, unit, onClose, onStart }: {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
      <div className="bg-[#0d1421] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="px-6 py-5 border-b border-white/[0.06] flex items-center justify-between">
          <div className="flex items-center gap-2"><Satellite className="w-5 h-5 text-[#2dd4bf]" /><h2 className="text-white font-bold">Avvia Analisi Rischio</h2></div>
          <button onClick={onClose} className="w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-slate-400 transition-colors"><X className="w-4 h-4" /></button>
        </div>
        <div className="p-6 space-y-4">
          {drawnArea && (
            <div className="flex items-center gap-3 p-3 bg-[#2dd4bf]/5 border border-[#2dd4bf]/20 rounded-xl">
              <SquareDashedBottom className="w-4 h-4 text-[#2dd4bf]" />
              <div>
                <p className="text-xs text-[#2dd4bf]/60 font-medium">Area selezionata</p>
                <p className="text-sm font-semibold text-white">{drawnArea.type === 'rectangle' ? 'Rettangolo' : drawnArea.type === 'lasso' ? 'Zona libera' : 'Poligono'} · {fmt(drawnArea.area, unit)}</p>
              </div>
            </div>
          )}
          <div>
            <label className="text-xs font-medium text-slate-400 block mb-2">Tipo analisi</label>
            <div className="grid grid-cols-2 gap-2">
              {([['snapshot', 'Snapshot', Camera, 'Situazione attuale'], ['timeseries', 'Serie Storica', TrendingUp, 'Trend nel periodo']] as const).map(([k, lbl, Icon, desc]) => (
                <button key={k} onClick={() => setMode(k)}
                  className={`flex flex-col items-start p-3 rounded-xl border-2 transition-all ${mode === k ? 'border-[#2dd4bf] bg-[#2dd4bf]/5' : 'border-white/10 hover:border-white/20'}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <Icon className={`w-4 h-4 ${mode === k ? 'text-[#2dd4bf]' : 'text-slate-500'}`} />
                    <span className={`text-xs font-semibold ${mode === k ? 'text-[#2dd4bf]' : 'text-slate-400'}`}>{lbl}</span>
                  </div>
          ...
          <div>
            <label className="text-xs font-medium text-slate-400 block mb-1.5">Nome analisi</label>
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Es. Zona industriale Nord" autoFocus
              className="w-full h-10 bg-white/5 border border-white/10 rounded-xl px-3 text-sm text-white placeholder-slate-600 outline-none focus:border-[#2dd4bf]/50 transition-all" />
          </div>
          {mode === 'timeseries' && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-slate-400 block mb-1.5">Inizio</label>
                <input type="date" value={startDate} max={endDate} onChange={e => setStartDate(e.target.value)}
                  className="w-full h-10 bg-white/5 border border-white/10 rounded-xl px-3 text-sm text-white outline-none focus:border-[#2dd4bf]/50 transition-all [color-scheme:dark]" />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-400 block mb-1.5">Fine</label>
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 h-11 bg-white/5 hover:bg-white/10 text-slate-300 text-sm font-medium rounded-xl transition-colors">Annulla</button>
            <button onClick={() => onStart(title || 'Analisi', startDate, endDate, mode)} disabled={!drawnArea}
              className="flex-1 h-11 bg-[#2dd4bf] hover:bg-[#14b8a6] disabled:opacity-30 text-slate-900 text-sm font-bold rounded-xl flex items-center justify-center gap-2 transition-colors">
...
function CoordDialog({ open, onClose, onStart }: {
  ...
  const inputCls = "w-full h-10 bg-white/5 border border-white/10 rounded-xl px-3 text-sm text-white placeholder-slate-600 outline-none focus:border-[#2dd4bf]/50 transition-all"
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
      <div className="bg-[#0d1421] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        <div className="px-6 py-5 border-b border-white/[0.06] flex items-center justify-between">
        ...
        <div className="p-6 space-y-4">
          ...
          {valid && (
            <div className="flex items-center gap-3 p-3 bg-[#2dd4bf]/5 border border-[#2dd4bf]/20 rounded-xl">
              <Navigation className="w-4 h-4 text-[#2dd4bf] flex-shrink-0" />
              <div>
                <p className="text-xs text-[#2dd4bf]/60">Area calcolata</p>
                <p className="text-sm font-semibold text-white">{area < 1 ? `${(area * 100).toFixed(1)} ha` : `${area.toFixed(2)} km²`} · {cLat.toFixed(4)}°N, {cLon.toFixed(4)}°E</p>
              </div>
            </div>
          )}
          <div className="grid grid-cols-2 gap-2">
            {([['snapshot', 'Snapshot', Camera], ['timeseries', 'Serie Storica', TrendingUp]] as const).map(([k, lbl, Icon]) => (
              <button key={k} onClick={() => setMode(k)} className={`flex items-center gap-2 p-2.5 rounded-xl border-2 text-xs font-semibold transition-all ${mode === k ? 'border-[#2dd4bf] bg-[#2dd4bf]/5 text-[#2dd4bf]' : 'border-white/10 text-slate-400 hover:border-white/20'}`}>
          ...
          {mode === 'timeseries' && (
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-medium text-slate-400 block mb-1.5">Inizio</label><input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className={inputCls + " [color-scheme:dark]"} /></div>
              <div><label className="text-xs font-medium text-slate-400 block mb-1.5">Fine</label><input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className={inputCls + " [color-scheme:dark]"} /></div>
            </div>
          )}
          <div className="flex gap-3 pt-1">
            <button onClick={onClose} className="flex-1 h-11 bg-white/5 hover:bg-white/10 text-slate-300 text-sm font-medium rounded-xl transition-colors">Annulla</button>
    ...
    <div className="relative group">
      <button onClick={onClick} disabled={disabled}
        className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all shadow-sm ${
          ...
          : 'bg-black/40 backdrop-blur text-slate-400 hover:bg-black/60 hover:text-white border border-white/10'
        }`}>{children}</button>
      {!disabled && (
        <div className="absolute left-full ml-3 top-1/2 -translate-y-1/2 bg-slate-900 border border-white/10 text-white text-xs font-medium px-2.5 py-1.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 shadow-lg">{tooltip}</div>
      )}
    </div>
  )
}
...
export function AppShell() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  return (
    <div className="h-screen w-screen flex bg-[#060d17] overflow-hidden" style={{ fontFamily: "'DM Sans', 'Inter', system-ui, sans-serif" }}>

      {/* ── SIDEBAR ───────────────────────────────────────────────────── */}
      <aside className="w-[215px] flex-shrink-0 bg-[#090f1c] border-r border-white/[0.06] flex flex-col">
        ...
        <div className="px-5 pt-6 pb-5 border-b border-white/[0.05]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-bold text-[15px] leading-none tracking-tight">GeoBridge</p>
            ...
            return (
              <button key={item.id} onClick={() => setView(item.id)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium transition-all relative group ${
                  active ? 'bg-[#2dd4bf]/10 text-[#2dd4bf]' : 'text-slate-500 hover:text-slate-200 hover:bg-white/[0.04]'
                }`}>
                <Icon className="w-[17px] h-[17px] flex-shrink-0" />
                <span className="flex-1 text-left">{item.label}</span>
                {item.badge ? (
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center ${
                    active ? 'bg-[#2dd4bf]/20 text-[#2dd4bf]' :
                    item.id === 'alerts' && alertCount > 0 ? 'bg-red-500/20 text-red-400' :
                    'bg-white/8 text-slate-400'
                  }`}>{item.badge}</span>
                ) : null}
                {active && <span className="absolute right-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-[#2dd4bf] rounded-l-full" />}
              </button>
            )
          })}
        </nav>
        ...
        <div className="px-3 pb-4">
          <button onClick={() => setView('map')}
            className="w-full h-10 bg-[#2dd4bf] hover:bg-[#14b8a6] text-slate-900 text-[13px] font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-[#2dd4bf]/15 hover:shadow-[#2dd4bf]/25">
        ...
        <div className="px-4 py-4 border-t border-white/[0.05]">
          <div className="flex items-center gap-2.5 mb-3">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#2dd4bf] to-[#0ea5e9] flex items-center justify-center flex-shrink-0 text-slate-900 text-xs font-bold">
              {userName.slice(0, 2).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-white text-xs font-semibold truncate">{userName}</p>
              <p className="text-[10px] text-slate-600 truncate">{isDemo ? 'Demo Mode' : user?.email}</p>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <div className={`w-1.5 h-1.5 rounded-full ${realtimeStatus === 'connected' ? 'bg-[#2dd4bf]' : isDemo ? 'bg-amber-400' : 'bg-slate-600'}`} />
              <span className="text-[10px] text-slate-600">{realtimeStatus === 'connected' ? 'Live' : isDemo ? 'Demo' : 'Offline'}</span>
            </div>
            <button onClick={() => signOut()} className="flex items-center gap-1 text-[10px] text-slate-600 hover:text-red-400 transition-colors">
      ...
      <main className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        {/* Top bar */}
        <header className="h-[56px] bg-[#090f1c]/90 backdrop-blur border-b border-white/[0.05] flex items-center px-6 gap-4 flex-shrink-0 z-10">
          <div className="flex-1 min-w-0">
            <h1 className="text-white font-semibold text-[14px] leading-none">{viewTitles[view]}</h1>
          ...
          {view === 'map' && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setShowCoordDialog(true)}
                className="flex items-center gap-1.5 h-8 px-3 bg-white/[0.04] hover:bg-white/[0.08] text-slate-300 text-xs font-medium rounded-lg transition-colors border border-white/[0.07]">
                <Navigation className="w-3.5 h-3.5" /> Coordinate
              </button>
              <div className="flex bg-white/[0.04] border border-white/[0.07] rounded-lg p-0.5">
                {(['street', 'satellite', 'topo'] as MapStyleKey[]).map(s => (
                  <button key={s} onClick={() => setMapStyle(s)}
                    className={`h-7 px-2.5 rounded-md text-[11px] font-medium transition-all ${mapStyle === s ? 'bg-[#2dd4bf] text-slate-900' : 'text-slate-500 hover:text-white'}`}>
          ...
          <button className="relative w-8 h-8 rounded-lg bg-white/[0.04] border border-white/[0.07] flex items-center justify-center text-slate-500 hover:text-white transition-colors">
        ...
        <div className="flex-1 overflow-hidden">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
                ...
                <div className="lg:col-span-3">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-white font-semibold text-sm">High Priority Alerts</p>
                  ...
                  <div className="space-y-2">
                    {loading ? (
                      ...
                      <div className="text-center py-10 bg-white/[0.02] border border-white/[0.05] rounded-2xl">
                        ...
                        <p className="text-slate-700 text-xs mt-1">Il portafoglio è nella norma</p>
                      </div>
                    ) : (
                ...
                <div className="lg:col-span-2">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-white font-semibold text-sm">Analisi Recenti</p>
                  ...
                  <div className="space-y-2">
                    {loading ? (
                      ...
                      analyses.slice(0, 5).map(a => {
                        ...
                        return (
                          <div key={a.id} onClick={() => router.push(`/analysis/${a.id}`)}
                            className="flex items-center gap-3 p-3 bg-white/[0.025] hover:bg-white/[0.045] border border-white/[0.06] rounded-xl cursor-pointer transition-all group">
                  ...
                  {/* Portfolio gauge */}
                  {analyses.length > 0 && (
                    <div className="mt-4 p-4 bg-white/[0.025] border border-white/[0.06] rounded-2xl">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="relative w-16 h-16 flex-shrink-0">
                          ...
                          <div className="absolute inset-0 flex flex-col items-center justify-center">
                            <span className="text-sm font-bold text-white tabular-nums">{avgRisk}%</span>
                          </div>
                        </div>
          ...
          {view === 'map' && (
            <div className="h-full relative">
              ...
              {drawMode && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-slate-950/90 backdrop-blur border border-white/10 text-white px-4 py-2 rounded-full text-xs flex items-center gap-3 pointer-events-none shadow-xl">
              ...
              <div className="absolute right-4 bottom-16 z-10 flex flex-col">
                <button onClick={() => mapRef.current?.zoomIn()} className="w-9 h-9 bg-[#090f1c]/90 border border-white/10 rounded-t-xl flex items-center justify-center text-slate-400 hover:text-white transition-colors"><Plus className="w-4 h-4" /></button>
                <button onClick={() => mapRef.current?.zoomOut()} className="w-9 h-9 bg-[#090f1c]/90 border border-white/10 rounded-b-xl border-t-0 flex items-center justify-center text-slate-400 hover:text-white transition-colors"><Minus className="w-4 h-4" /></button>
              </div>

              {/* Area panel */}
              {(drawnArea || lastAnalyzedArea || searchResult) && (
                <div className="absolute top-4 right-4 z-20 w-68">
                  <div className="bg-[#090f1c]/95 backdrop-blur border border-white/10 rounded-2xl shadow-2xl overflow-hidden" style={{ width: 268 }}>
                    <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
                      <span className="text-white text-sm font-semibold">{lastAnalyzedArea && !drawnArea ? 'Ultima area' : 'Area selezionata'}</span>
                      <button onClick={() => { setDrawnArea(null); setLastAnalyzedArea(null); setSearchResult(null); mapRef.current?.clearDrawing() }}
                        className="w-6 h-6 rounded-lg bg-white/5 flex items-center justify-center text-slate-500 hover:text-white transition-colors"><X className="w-3.5 h-3.5" /></button>
                    </div>
                    <div className="p-4 space-y-3">
                      ...
                      {(drawnArea || lastAnalyzedArea) && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-[#2dd4bf]">{fmt((drawnArea || lastAnalyzedArea)!.area, settings.unit)}</span>
                        </div>
                      )}
                      <button onClick={() => setShowAnalysisModal(true)} disabled={!drawnArea}
                        className="w-full h-10 bg-[#2dd4bf] hover:bg-[#14b8a6] disabled:opacity-30 text-slate-900 text-sm font-bold rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg shadow-[#2dd4bf]/10">
              ...
              {/* Status */}
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-10 bg-[#090f1c]/90 backdrop-blur border border-white/[0.08] text-white px-4 py-2 rounded-full text-xs flex items-center gap-2 pointer-events-none">
            ...
            <div className="h-full overflow-y-auto p-6">
              ...
              {loading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-slate-400 text-sm font-medium">Nessun report disponibile</p>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {analyses.map(a => {
                      ...
                      return (
                        <div key={a.id}
                          className={`group relative bg-white/[0.025] hover:bg-white/[0.045] border rounded-2xl overflow-hidden transition-all cursor-pointer ${isSelected ? 'border-[#2dd4bf]/30 shadow-lg shadow-[#2dd4bf]/5' : 'border-white/[0.07] hover:border-white/15'}`}
                          ...
                          <div className="p-4">
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex-1 min-w-0 pr-2">
                                ...
                                <p className="text-[11px] text-slate-600 mt-0.5 truncate">{a.address?.split(',')[0]} · {a.startDate?.slice(0, 7)}</p>
                              </div>
                              <RiskBadge level={a.compositeLevel} />
                            </div>
                            <div className="grid grid-cols-4 gap-2 mb-3">
                              {(a.categories || []).slice(0, 4).map(cat => (
                                <div key={cat.name} className="text-center bg-white/[0.03] rounded-lg py-1.5">
                                  <p className="text-[9px] text-slate-600 truncate px-0.5">{cat.name.split(' ')[0]}</p>
                                  <p className="text-xs font-bold text-white">{cat.score}</p>
                                </div>
                              ))}
                            </div>
                            ...
                            <div className="flex items-center justify-between" onClick={e => e.stopPropagation()}>
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button onClick={() => generateAnalysisPDF(a)} className="w-6 h-6 rounded-lg bg-white/5 flex items-center justify-center text-slate-500 hover:text-white transition-colors"><Download className="w-2.5 h-2.5" /></button>
            ...
            <div className="h-full overflow-y-auto p-6">
              {selectedForComparison.size < 2 ? (
                <div className="text-center py-20">
                  ...
                  <button onClick={() => setView('reports')} className="h-9 px-5 bg-white/[0.05] border border-white/10 text-slate-300 text-sm rounded-xl hover:bg-white/10 transition-colors">Vai a Reports</button>
                </div>
              ) : (
          ...
          {view === 'apikeys' && (
            <div className="h-full overflow-y-auto p-6">
              <div className="max-w-xl">
                ...
                <div className="bg-[#090f1c] border border-white/[0.07] rounded-2xl overflow-hidden">
                  <ApiKeysPanel />
                </div>
              </div>
            </div>
            ...
            <div className="h-full overflow-y-auto p-6">
              <div className="max-w-md space-y-4">
                <div className="bg-white/[0.025] border border-white/[0.07] rounded-2xl p-5">
                  <h3 className="text-sm font-semibold text-white mb-4">Unità di misura</h3>
                  <div className="flex gap-2">
                    {(['km2', 'ha'] as const).map(u => (
                      <button key={u} onClick={() => setSettings(s => ({ ...s, unit: u }))}
                        className={`h-9 px-4 rounded-xl text-sm font-medium transition-all ${settings.unit === u ? 'bg-[#2dd4bf] text-slate-900' : 'bg-white/5 text-slate-400 border border-white/10 hover:text-white'}`}>
                ...
                <div className="bg-white/[0.025] border border-white/[0.07] rounded-2xl p-5">
                  <h3 className="text-sm font-semibold text-white mb-4">Stile mappa predefinito</h3>
                  <div className="flex gap-2">
                    {(['street', 'satellite', 'topo'] as MapStyleKey[]).map(s => (
                      <button key={s} onClick={() => setSettings(st => ({ ...st, defaultMap: s }))}
                        className={`h-9 px-4 rounded-xl text-sm font-medium transition-all ${settings.defaultMap === s ? 'bg-[#2dd4bf] text-slate-900' : 'bg-white/5 text-slate-400 border border-white/10 hover:text-white'}`}>
                ...
                <div className="bg-white/[0.025] border border-white/[0.07] rounded-2xl p-5">
                  ...
                  <p className="text-sm text-slate-400 mb-1">{user?.email || 'Demo'}</p>
                  {isDemo && <p className="text-xs text-amber-400/80 mb-3">Modalità demo — dati in locale</p>}
